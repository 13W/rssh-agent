...
async fn test_connection_stats() {
    ...
    println!(
        ...
        stats.average_response_time_ms.load(Ordering::Relaxed)
    );
}
...
async fn test_cache_memory_cleanup() {
    ...
    assert_eq!(
        present_count, 2,
        "Cache should limit to 2 keys and evict excess"
    );
...
fn test_performance_baseline() {
    ...
    assert!(
        ...
        "Basic allocation should be fast"
    );
}
